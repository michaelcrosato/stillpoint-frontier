...
- `npm run test:e2e` — browser boot, WebGL pixel, streaming, interaction, and context-loss tests.
...
automation, and exposes only snapshot, teleport, fixed view, discovery, and WebGL
context-loss operations. Normal gameplay does not depend on the bridge.
