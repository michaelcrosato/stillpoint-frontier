...
| --- | --- | --- |
| Merge pipeline | Branch protection required `Analyze (actions)` and `Analyze (javascript-typescript)`. CodeQL default setup emits those per-language checks on ordinary pull requests, but on Dependabot pull requests it emits a single `CodeQL` check with conclusion `neutral` and no `Analyze` checks at all, because the restricted Dependabot token cannot upload results. The required contexts therefore never appeared on a Dependabot pull request, and with `enforce_admins` on, **every Dependabot pull request was permanently unmergeable** while ordinary ones merged normally. Eight had accumulated, the oldest three weeks, including the fix for two critical advisories. Verified against the check runs for #7 (ordinary: both `Analyze` checks succeeded) and #10 (Dependabot: `CodeQL` neutral, no `Analyze`). | The two contexts were removed from required status checks. Requiring them is incompatible with merging Dependabot pull requests at all, so this is the trade the situation forces; CodeQL still runs on every pull request and still reports to the Security tab, and there are currently no open code-scanning alerts. `CodeQL` was deliberately not required in their place, since its `neutral` conclusion on Dependabot pull requests would reintroduce the same failure. |
...
- **`installTestBridge` ships in the client bundle**, runtime-gated on `?test=1`.
  ...
  and `storageEnabled: !testMode` means the bridge cannot write a survey save —
  and the e2e suite depends on it, so build-time stripping would cost that
  coverage. Recorded rather than changed.
